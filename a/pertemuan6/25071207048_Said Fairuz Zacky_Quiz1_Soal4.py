...
def matriksMenu(menu):
    while True:
        try:
            hari = int(input("Masukan Banyak Hari: "))
            menu = int(input("Masukan Banyak Menu: "))
            break
        except ValueError:
            print("[Error] Masukan Hanya Angka!")
    matriks = [[0 for x in range(hari)] for j in range(menu)]
    for i in range(hari):
        print(f"Hari Ke-{i+1}")
        print("=" * 15)
        for j in range(menu):
            menu = int(input(f"Menu ke-{j+1}: "))
            matriks[j][i] = menu
        print()
